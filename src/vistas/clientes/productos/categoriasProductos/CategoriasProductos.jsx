...
const ItemServicios = ({ item: { subCategorias, visibilidad, categoryId, unidades, id, name, description, price, img, volumen, genero, referencia } }) => {
    ...
    return (
        <>
            {visibilidad === 'ACTIVO' ? (
                <Link to={`/producto-filtro-categoria/${categoryId}`} style={{ textDecoration: 'none' }}>
                    <div className="divImagenCategoriasCentro">
                        {images.length ? (
                            <div className="imagenContainer">
                                <img className="imagenesCategorias" src={img[0]} alt={name} />
                                <div className="divTexMovido" 
                                    style={{ position: "relative" }}>
                                <span
                                    className="textoMovido">
                                    {name}
                                </span>
                                </div>
                            </div>
