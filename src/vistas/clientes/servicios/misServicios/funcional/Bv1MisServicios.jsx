              ...
              {servicios.map((task) => (
                <div className="divFichaServicioCliente" key={task.id}>
                  ...
                  <div
                    ...
                    style={{ margin: "0px 0px 0px 0px" }}
                  >
                    {task.timeMs.length < task.cartItem[0].count ? (
                      <div className="alineacionVertical" style={{ margin: "10px 0px 10px 0px" }}>
                        {isWithinSevenDays(task.timeMs[task.timeMs.length - 1]) ? (
                          <div className="divMensajePorCuidado" >
                            <span className="textoPorCuidado">
                              ¡Por el cuidado de tu piel debes
                              <br />
                               agendar cuando pasen {calculateDaysRemaining(task.timeMs[task.timeMs.length - 1])} días!
                            </span>
                          </div>
                        ) : (
                            <div className="boton2" onClick={() => { providerIdParaActualizar(task.id) }}>
