...
function SiguienteHoraServicio() {
  //console.log('llamando a SiguienteHoraServicio');
  const {
  ...
  const [pruebaduracionCita, setpruebaduracionCita] = useState(30);

  //console.log(contextDiaDinamicoSeleccionado, "contextDiaDinamicoSeleccionado");
  ...
  const fCalcularHorarios = (inJoMs, fiJoMs, inDeMs, fiDeMs) => {
    ...
    let tiempoCitas = pruebaduracionCita * 60000;
 ...
 const [revisionHorariosBloqueados, setrevisionHorariosBloqueados] = useState("");
  ...
  const fCalcularHorariosBloqueados = (inBloqMs, fiBloqMs) => {
    const tiempoCitas = pruebaduracionCita * 60000;
    const listaHorasMilisegundos = [];
  ...
  return (
    <div className="alineacionVertical">
      <div className="divSalirvolverProductosDetallados">
        <Link to="/siguiente-dia-servicio">
          <HiArrowNarrowLeft
            style={{
              fontSize: "35px",
...
export default SiguienteHoraServicio;
