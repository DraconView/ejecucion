...
function HorasDisponiblesDinamicas() {
 //console.log('llamando a HorasDisponiblesDinamicas');
  const {
  ...
  useEffect(() => {
    // setea el horario de la mañana y de la tarde con las reglas
    let joSet = new Date(contextDiaDinamicoSeleccionado);
    let inJoDate = new Date(joSet);
    // reglas[0] es el primer objeto del array reglas
    inJoDate.setHours(
      reglas[0].inJo[0],
      reglas[0].inJo[1],
      reglas[0].inJo[2],
