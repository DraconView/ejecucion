...
function ConsultaCiudades() {
  ...
  useEffect(() => {
    if (contextCiudades) {
      setciudades(contextCiudades.map((ciudad) => ciudad.nombrePublico)); {/* "no cambiar" maneja  la ciudad */}
...
if (ciudad) {
  return (
    <>
      <EditarDiasCalendario ciudad={ciudad} />
    </>
  );
}
    ...
    <div>
      ...
      {ciudadesMasDias.map((ciudad, index) => (
        <div key={index}
          ...
          onClick={() => {
            setciudad(ciudad.ciudad); {/* "no cambiar" actualiza la ciudad */} 
            providerEleccionCiudad(ciudad.ciudad); { /* "no cambiar" actualiza la ciudad */ }
          }}
        >
          {ciudad.ciudad}: {/* "no cambiar" muestra la ciudad */}
          <span
            style={{
