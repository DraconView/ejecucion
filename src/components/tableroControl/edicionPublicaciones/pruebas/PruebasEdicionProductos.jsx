// crud con funciones en coleccion segun criterio
...
import { addDocument, deleteDocument, getCollection, updateDocument  } from '../../../actions/AccionesbdFire' 
...
const EdicionProductos = () => {  
  ...
  useEffect(() => {
    (async () => {
        const result = await getCollection(`${criterioBusqueda}`)
  ...
  const addTask = async(e) => {
    ...
    const result = await addDocument(`${criterioBusqueda}`, { name: name }) //> adicionando publicacion a la coleccion de bdProductos  
    if (!result.statusResponse) {
  ...
  const deleteTask = async(id) => {
    const result = await deleteDocument(`${criterioBusqueda}`, id) //> bdProductos indica de cual coleccion va eliminar con el ID que se le pase por parametro 
        ...
        <div className="alineacionBarraDeBusquedaEdicionProductos"
          ...
          <BiSearchAlt 
              style={{ position: 'relative', left: 35, top: 18, fontSize:'25px', 
                       color:'#FEBA20'}}/>
          <input
              placeholder="Categoria del producto..." value={criterioBusqueda.toUpperCase()} 
            ...
            <div className="estiloListaEdicion" >
           {
            ...
            <div className="alineacionHorizontalEdicion" key={task.id}>
              ...
              <img
                ...
                src={imagesArray.map(img => <img src={img} 
