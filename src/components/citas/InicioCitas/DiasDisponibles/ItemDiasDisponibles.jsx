...
const ItemDiasDisponibles = ({ item: { fechaMesEscrito, stock, fechaNavegacion, fechaLegible, subCategorias, visibilidad, categoryId, unidades, id, name, description, price, img, volumen, genero, referencia } }) => {
    ...
    return (
        <>
        {stock === 1 ?
        <Link to={`/confirmacion-fecha-hora/${fechaNavegacion}`} style={{ textDecoration: 'none' }}>
            <div className="divFechaDisponible" >
                <span >{fechaMesEscrito}</span>
            </div>
        </Link>
        : null}
        </>
...
export default ItemDiasDisponibles
